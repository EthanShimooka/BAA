...
const char* kGameName = "birdsatarms";
...
void GamerServices::Impl::OnLobbyEnteredCallback(LobbyEnter_t* inCallback, bool inIOFailure)
{
	...
	else
	{
		...
		log->logBuffer << "Error 401: Failed to join lobby\n";
		log->flush();
	...
	LeaderboardScoresDownloaded_t* inCallback, bool inIOFailure)
{
	if (!inIOFailure)
	{
		...
		for (int i = 0; i < inCallback->m_cEntryCount; ++i)
		{
			...
			LogManager* log = LogManager::GetLogManager();
			log->logBuffer << "%d. %s = %d", entry.m_nGlobalRank,
...
bool GamerServices::StaticInit()
{
	...
	{
		LogManager* log = LogManager::GetLogManager();
		log->logBuffer << "Error 402: Failed to initialize Steam. Make sure you are running a Steam client.\n";
		log->flush();
	}
	else
	{
