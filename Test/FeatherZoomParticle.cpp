...
void FeatherZoomParticle::Update(){
	...
	if (alive && ((currenttime - lasttime) > frequencyOfParticles || oldTime%frequencyOfParticles > progress%frequencyOfParticles)){
		...
		for (unsigned int i = 0; i < numberOfParticles; i++){
			...
			movements.push_back(makeMotion(resizeSquareTransform(sprite, 0.01,0.5), 0.0, 0.5, ease_QuadOut));
			movements.push_back(makeMotion(resizeSquareTransform(sprite, 0.5, 0.01), 0.5, 1.0, ease_QuadIn));
